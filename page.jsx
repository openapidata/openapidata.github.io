  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] text-slate-200 font-sans selection:bg-indigo-500/30 flex flex-col">
      ...
      <nav className="border-b border-slate-800/60 bg-[#0B0F19]/80 backdrop-blur-md sticky top-0 z-50">
        ...
        {/* Mobile Nav Dropdown */}
        {mobileMenuOpen && (
           <div className="md:hidden border-t border-slate-800 bg-[#0B0F19] px-4 py-4 space-y-4 shadow-2xl">
              <button 
                onClick={() => { setActiveTab('docs'); setMobileMenuOpen(false); }} 
                className={`block w-full text-left py-2 ${activeTab === 'docs' ? 'text-white font-bold' : 'text-slate-400'}`}
              >
                Documentation
              </button>
              <button 
                onClick={() => { setActiveTab('playground'); setMobileMenuOpen(false); }} 
                className={`block w-full text-left py-2 ${activeTab === 'playground' ? 'text-white font-bold' : 'text-slate-400'}`}
              >
                Playground
              </button>
              <a href="https://github.com/openapidata/openapidata.github.io" target="_blank" rel="noreferrer" className="block w-full text-left py-2 text-slate-400">
                GitHub Repo
              </a>
           </div>
        )}
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16 md:py-24 relative z-10">
              <div className="max-w-3xl">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <button onClick={() => setActiveTab('playground')} className="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-3.5 rounded-lg font-semibold transition-all shadow-lg shadow-indigo-900/20 flex items-center justify-center gap-2 group">
                    <Zap className="w-4 h-4 fill-current group-hover:scale-110 transition-transform" /> Try Playground
                  </button>
                  <button onClick={() => document.getElementById('quickstart').scrollIntoView({ behavior: 'smooth'})} className="bg-slate-800 hover:bg-slate-700 text-white px-8 py-3.5 rounded-lg font-semibold transition border border-slate-700 flex items-center justify-center gap-2">
                ...
                <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6 lg:sticky lg:top-24">
                  ...
                  <div className="space-y-4 text-sm text-slate-400">
                    <div className="flex flex-col sm:flex-row sm:justify-between py-2 border-b border-slate-800/50 gap-1 sm:gap-0">
                      <span>Base URL</span>
                      <code className="text-indigo-300 break-all">openapidata.github.io/api/v1</code>
                    </div>
        ...
        <footer className="border-t border-slate-800/60 bg-[#0B0F19] py-12 mt-auto">
          <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div className="flex gap-4">
               <a target="_blank" href="https://github.com/openapidata/openapidata.github.io" className="text-slate-500 hover:text-indigo-400 transition">GitLab</a>
